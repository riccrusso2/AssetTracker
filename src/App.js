...
const PhysGoldModal = ({ physGold, onSave, onClose }) => {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      costBasisPerGram: parseFloat(form.costBasisPerGram) || 0,
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <label className="field-label">Prezzo medio carico (€/g)
            <input type="number" step="any" min="0" value={form.costBasisPerGram}
              onChange={(e) => set("costBasisPerGram", e.target.value)} className="field-input"/>
          </label>
          <label className="field-label">
          ...
          <p className="hint-text" style={{ marginTop: 0 }}>
            Il prezzo 18kt viene calcolato come <strong>prezzo spot oro (€/g) × 0,75</strong>, 
            aggiornato via <code>/api/gold-price</code>. 
...
export default function App() {
  ...
  const fetchGoldSpotPrice = useCallback(async () => {
    // Expects backend /api/gold-price to return:
    //   { spotEurPerTroyOz: number } OR { spotEurPerGram: number }
    // 18kt = 75% pure gold
    const res = await fetch("/api/gold-price");
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    const data = await res.json();
    let pricePerGram = null;
    if (typeof data.spotEurPerGram === "number") {
      pricePerGram = data.spotEurPerGram;
    } else if (typeof data.spotEurPerTroyOz === "number") {
      pricePerGram = data.spotEurPerTroyOz / 31.1035;
    } else {
      throw new Error("Formato risposta /api/gold-price non valido");
    }
    const price18kt = r2(pricePerGram * 0.75);
    // Only overwrite if no manual price has been set in the modal
    setPhysGold((prev) => {
      // If the user typed a manual price in the modal, don't overwrite it
      return { ...prev, pricePerGram18kt: price18kt, lastUpdated: new Date().toISOString() };
    });
  }, []);
  ...
  const physGoldValue = useMemo(() =>
    ...
    [physGold]);
  const physGoldPerfE   = physGold.pricePerGram18kt && physGold.costBasisPerGram && physGold.grams
    ? r2((physGold.pricePerGram18kt - physGold.costBasisPerGram) * physGold.grams) : 0;
  const physGoldPerfPct = physGold.pricePerGram18kt && physGold.costBasisPerGram
    ? r2(((physGold.pricePerGram18kt - physGold.costBasisPerGram) / physGold.costBasisPerGram) * 100) : 0;
            ...
            <>
                ...
                <table className="data-table">
                  <thead>
                    <tr>
                      ...
                      <th className="num">P. Acquisto /g</th>
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td className="num mono">
                        {physGold.costBasisPerGram > 0 ? fmt(physGold.costBasisPerGram) : <span className="muted">—</span>}
                      </td>
                      <td className="num mono">
                      ...
                      <td className={`num mono ${physGoldPerfE >= 0 ? "pos-text" : "neg-text"}`}>
                        {physGold.pricePerGram18kt && physGold.costBasisPerGram
                          ? `${physGoldPerfE >= 0 ? "+" : ""}${fmt(physGoldPerfE)}` : "—"}
                      </td>
                      <td className="num">
                        {physGold.pricePerGram18kt && physGold.costBasisPerGram
                          ? <Badge value={physGoldPerfPct}/> : "—"}
                      </td>
              ...
              {physGold.grams > 0 && !physGold.pricePerGram18kt && (
                <p className="hint-text">
                  ⚠ Prezzo 18kt non disponibile. Il backend deve esporre{" "}
                  <code style={{ background: "var(--bg-card2)", padding: "1px 5px", borderRadius: 4, fontSize: 11 }}>/api/gold-price</code>{" "}
                  con <code style={{ background: "var(--bg-card2)", padding: "1px 5px", borderRadius: 4, fontSize: 11 }}>{"{ spotEurPerTroyOz: number }"}</code> oppure inserisci il prezzo manualmente dalla matita.
                </p>
              )}
              {physGold.grams > 0 && physGold.pricePerGram18kt && (
                <p className="hint-text" style={{ color: "var(--text-muted)" }}>
                  Prezzo spot calcolato come <strong>prezzo oro EUR/oz ÷ 31,1035 × 0,75</strong>.
                  {physGold.lastUpdated && (
